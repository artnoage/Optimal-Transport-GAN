    ...
    def train(self, n_critic_loops=None, n_main_loops=None, n_assign_loops=None):
        with self.session as session:
            ...
            for main_loop in tqdm.tqdm(range(global_step, n_main_loops, 1)):
                ...
                fakes = session.run(self.model.get_fake_tensor(), {self.model.latent_batch_ph: latent_sample_big[50:68]})
                reals = self.dataset.data[real_idx_big[50:68]]
...
def main():
    ...
    assignment_training = AssignmentTraining_new_approach(dataset=Mnist32(batch_size=100,dataset_size=300),
                                                          latent=VaiosLatent2(shape=20, batch_size=100),
    ...
    assignment_training.train(n_main_loops=1000, n_critic_loops=20, n_assign_loops=10)
