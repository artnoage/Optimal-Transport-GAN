    ...
    def train(self, n_critic_loops=None, n_main_loops=None):
        with self.session as session:
            ...
            for main_loop in tqdm.tqdm(range(global_step, n_main_loops, 1)):
                local_dataset_s = int(min((self.dataset.dataset_size)*(2*main_loop/n_main_loops)+200, self.dataset.dataset_size))
                assignment_loops = int(10*local_dataset_s / self.latent.batch_size) + 10
                if main_loop<200:
                    croop=12
                else:
                    croop=2
                with tqdm.tqdm(range(croop)) as crit_bar:
                ...
                self.model.train_generator(session, real_idx,latent_sample,offset=4)
...
def main():
    ...
    assignment_training = AssignmentTraining(dataset=Fashion32(batch_size=5000, dataset_size=5000),
                                             latent=Assignment_latent(shape=300, batch_size=400),
                                             ...
                                             cost="square")
    assignment_training.train(n_main_loops=1200, n_critic_loops=3)
