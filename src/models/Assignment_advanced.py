...
class Assignment_model:
    ...
    def define_graph(self):
        ...
        self.dataset_local_ph= tf.placeholder(shape=(None, self.dataset.get_total_shape()), dtype=tf.float32, name="real")
    ...
    def find_assignments_critic(self,session, assign_loops=100,local_dataset_size=200):
        ...
        if ratio-int(ratio)==0:
            ...
            ratio=int(ratio)+1
        assign_arr = np.zeros(shape=(local_dataset_size,))
    ...
    def assignment_critic_cost(self):
        ...
        crit_cost = -(
                ...
                - tf.reduce_mean(self.crit_network.tensor(self.dataset_local_ph)))
        return crit_cost
    ...
    def assignment_generator_cost_ssim(self):
        ...
        if self.dataset.shape[2] == 3:
            gen_cost = tf.reduce_mean(1 - tf.image.ssim_multiscale(tf.reshape(self.real_batch, batch_image_shape) + 1,
    ...
    def train_critic(self,session,assign_arr,local_dataset_size):
        ...
        samples.update({self.assign_samples_ph: self.dataset.data[assign_idx_local],
                        ...
                        self.dataset_local_ph:self.dataset.data[np.arange(0,local_dataset_size)]})
