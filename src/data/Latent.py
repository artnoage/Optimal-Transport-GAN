...
# With this class we generate random points in our latent space. The points are taken from N Gaussians.
# If one makes N too big then we have a perfect fit (or an overfit depending how one sees it).
# If the number of Gaussians is small then it is harder to train and someone has to increase the number of critic steps to avoid. However this way we observe
# the generation of more "new" points. We prefer this method from the standard one, because we believe that quite often the data manifold has different geometry in different parts.
...
class Assignment_latent(Latent):
    ...
    def tensor(self):
        ...
        return a+ 0.1*tf.random_normal(shape=(self.batch_size, self.shape))
